...
class Work(db.Model):
    ...
    cnc = db.Column(db.String(20), nullable=False, default="CNC1")  # DB'de yoksa auto-migrate
    ...
    process_time_sec = db.Column(db.Integer, nullable=False)  # parça başına saniye
    quantity = db.Column(db.Integer, nullable=False)

    downtime_reason = db.Column(db.String(200))
    downtime_seconds = db.Column(db.Integer, default=0)

    efficiency = db.Column(db.Float, nullable=False)  # admin görür, operator görmez
...
def calculate_break_overlap_seconds(start_dt: datetime, end_dt: datetime) -> int:
    total = 0
    for b_start, b_end in get_breaks():
        bs = parse_hhmm(b_start)
        be = parse_hhmm(b_end)
...
def calculate_net_seconds(start_str: str, end_str: str, downtime_seconds: int = 0) -> int:
    """
    Net süre = (end-start) - mola çakışmaları - kayıp zaman
    """
    ...
    breaks_sec = calculate_break_overlap_seconds(start_dt, end_dt)
    net = total - breaks_sec - max(int(downtime_seconds or 0), 0)
    return max(net, 0)
...
def calculate_efficiency_seconds(
    ...
    """
    Verim% = (Gerçek Adet / Hedef Adet) * 100
    Hedef Adet = Net Süre / (Process + Setup)
    """
    net_seconds = calculate_net_seconds(start_time, end_time, downtime_seconds=downtime_seconds)
    if net_seconds <= 0:
        return 0.0
...
def get_part_codes():
    parts = (
        Part.query.with_entities(Part.part_code)
        .distinct()
...
def login():
    if request.method == "POST":
        ...
        return render_template("login.html", message="Hatalı giriş!")
...
def operator_panel():
    ...
    if request.method == "POST":
        ...
        if operation not in OPERATIONS:
            ...
            # AYAR: verim hesaplama yok, parça zorunlu değil
            if operation == "Ayar":
                w = Work(
                    ...
                    end_time=end_time,
                    process_time_sec=max(process_sec, 0),
                    quantity=max(quantity, 0),
                    downtime_reason=downtime_reason if downtime_reason else None,
                    downtime_seconds=max(downtime_seconds, 0),
                    efficiency=0.0
                )
                db.session.add(w)
                db.session.commit()
                ...
                # Diğer operasyonlar: parça + operasyon admin tanımlı olmalı
                ...
                if not part:
                    ...
                    eff = calculate_efficiency_seconds(
                        start_time=start_time,
                        end_time=end_time,
    ...
    # Operatör sadece kendi kayıtlarını görür (verim gösterilmeyecek)
    works = (
        Work.query
        .filter_by(operator=session["user"])
        .order_by(Work.date.desc())
        .limit(100)
        .all()
    )

    # Ekranda “Net Süre” göstermek için hesaplayıp objeye ekliyoruz
    for w in works:
        w.net_seconds = calculate_net_seconds(w.start_time, w.end_time, w.downtime_seconds)
        w.net_human = seconds_to_hhmmss(w.net_seconds)
        w.proc_human = seconds_to_hhmmss(w.process_time_sec)
...
def admin():
                ...
                try:
                    ...
                    for rc in required_cols:
                        if rc not in headers:
                            raise ValueError(f"Excel başlığı eksik: {rc}")
    ...
    # Admin ekranda net süre gösterimi
    for w in works:
        w.net_seconds = calculate_net_seconds(w.start_time, w.end_time, w.downtime_seconds)
        w.net_human = seconds_to_hhmmss(w.net_seconds)
        w.proc_human = seconds_to_hhmmss(w.process_time_sec)
...
def ensure_cnc_column_if_missing():
    """
    Render Postgres / Local SQLite için:
    Work tablosunda cnc kolonu yoksa ekler.
    """
...
def ensure_db_and_admin():
    with app.app_context():
        ...
        # cnc kolonunu ekle (yoksa)
        try:
            ensure_cnc_column_if_missing()
        except Exception:
            pass

        # İlk kurulum admin (opsiyonel)
        create_admin = (os.environ.get("ADMIN_CREATE", "false").lower() == "true")
        admin_username = os.environ.get("ADMIN_USERNAME", "admin")
        admin_password = os.environ.get("ADMIN_PASSWORD")

        if create_admin and admin_password and len(admin_password) >= 6:
